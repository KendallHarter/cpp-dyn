...
static constexpr auto owner2 = khct::dyn<const noise_trait, khct::non_owning_options{.store_vtable_inline = true}>(&d);
...
// One pointer to the data, 5 pointers to methods, 1 pointer to destructor function
static_assert(sizeof(owner2) == sizeof(void*) + sizeof(void*) * 5 + sizeof(void*));

consteval
{
   cow cow2{};
   auto trait = khct::dyn<noise_trait>(&cow2);
   auto trait2 = khct::dyn<noise_trait, khct::non_owning_options{.store_vtable_inline = true}>(&cow2);
